...
class Session(BigIdAbstract):
    ...
    activity = models.JSONField(null=True, default=dict)
...
class Boo(BigIdAbstract, ModelWithFlag):
    ...
    @classproperty
    def guestboo(cls):
        return Boo.objects.get(id=GUESTBOO)
...
class Post(BigIdAbstract, ModelWithFlag):
    ...
    def vote(self, action, boo, note=''):
        ...
        if action==VOTE_UP:
            ...
            # try:
            #     Flager.objects.create(user=boo, linked_object=self, status=VOTE_UP, note=note)
            # except:
            #     pass
            #
            # try:
            #     Flager.objects.get(user=boo, content_type=ctype, object_id=self.id, status=VOTE_DOWN, note=note).delete()
            # except:
            #     pass
        ...
        # down
            ...
            # try:
            #     Flager.objects.create(user=boo, linked_object=self, status=VOTE_DOWN, note=note)
            # except:
            #     pass
            #
            # try:
            #     Flager.objects.get(user=boo, content_type=ctype, object_id=self.id, status=VOTE_UP, note=note).delete()
            # except:
            #     pass
