...
#     {id}
#     {img_id}
#     <img id='{img_id}' src='{image_url}' style='display:{display};height:auto;width:200px;margin-left:170px;' alt='{file_name}'>
#
#     <script>
#       function {fn_name}(event) {{
#         console.log(document.getElementById('{img_id}').parentElement);
...
#       //console.log(document.getElementById("{img_id}").parentElement.querySelector('input'));
#       document.getElementById("{img_id}").parentElement.querySelector('input').addEventListener('change', {fn_name}, false);
#     </script>'''
#
#
...
#             display = 'none'
#
#         img_id = 'img_' + str(datetime.now().timestamp()).replace('.', '_')
#         id = str(attrs['id'])
#         # fn_name = 'fn_' + id.replace('-', '_')
#         fn_name = 'fn_' + str(datetime.now().timestamp()).replace('.', '_')
#         output.append(super().render(name, value, attrs))
#         output.append(preview_template.format(id=id, image_url=image_url, file_name=file_name, display=display, fn_name=fn_name, img_id=img_id))
...
class ResearchItemAdmin(admin.ModelAdmin):
    list_display = ['research', 'order', 'type', 'question']
...
class ResearchAdmin(admin.ModelAdmin):
    ...
    list_display = ['title', 'owner', 'published', 'coverpix_preview']
    raw_id_fields = ('owner',)
    formfield_overrides = { models.ImageField: {'widget': AdminpixPreviewWidget} }
